...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log('[Verify Mobile] Verifying mobile for user:', user.strigaUserId)

    // Verify mobile with Striga
    const response = await strigaApiRequest<{ message: string }>('/user/verify-mobile', {
      method: 'POST',
      body: JSON.stringify({
    ...
    console.log('[Verify Mobile] Success:', response)
