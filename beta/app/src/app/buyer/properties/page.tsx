...
export default function BuyerPropertiesPage() {
  ...
  const handleInterest = async (propertyId: string) => {
    try {
      ...
      if (response.ok) {
        // Refresh properties to update interest status
        fetchProperties()
      }
    } catch (err) {
  ...
  if (status === 'loading' || (status === 'authenticated' && session?.user?.role !== 'BUYER')) {
    return (
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 mt-2">Find your perfect property in Portugal</p>
            </div>
            ...
            {properties.map((property) => (
              <Card key={property.id} className="hover:shadow-lg transition-shadow">
                ...
                <CardFooter className="flex gap-2">
                  <Link href={`/property/${property.code}`} className="flex-1">
